...
```{r table part 1, message=FALSE, warning=FALSE, include=TRUE}
library(formattable)
...
A few more steps with this one, but worth it for the final product
...
`r emo::ji("coin")` It's **free** (desktop version)

--

`r emo::ji("backhand index pointing right")` All calculations/formulas can be done directly in **Power BI**, just connect/import data
...
knitr::include_graphics("Images/Power Bi.png")
...
.pull-left[
Create a training drill intensity report also using R

Intensity of each drill/player compared to the peak of a game for 3 metrics 
...
.pull-right[
```{r power bi report 2, echo=FALSE, fig.align='center', out.width='160%'}
library(pdftools)
magick::image_read_pdf("Images/Drill Intensity Breakdown.pdf", pages = 1)
...
Repeat similiar steps using **catapultR** 

Added in **period name** (drill name)
...
The calculation simply determines the % intensity a drill is compared to the peak of a game by factoring in the duration of the drill


Add a table and select columns you want (averages)


Replicate table but add **player** 
...
```{r power bi rules, echo = FALSE, fig.align='center', out.width='90%'}
knitr::include_graphics("Images/Power BI formatting rules.jpg")
