...
```{r imbalance, echo=TRUE, message=FALSE, warning=FALSE}
Nordboard %>%
            filter(Player == "Player 10") %>% 
...
```{r imbalance1, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, fig.retina=1, message=FALSE, warning=FALSE}
Nordboard %>%
            filter(Player == "Player 10") %>% 
            filter(Variable == "Max Force Imbalance") %>%
          mutate(Direction = ifelse(value > 0,"Right", "Left")) %>%
